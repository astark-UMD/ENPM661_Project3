...
def A_star_search(map, obstacles, start, end, sf, step_size):
    ...
    while open_set:
        ...
        if closed_set[int(current_y/.5), int(current_x/.5), angle_to_index(current_theta)] == True:
            continue
        else: 
            # Add the current node to the closed set.
            closed_set[int(current_y/.5), int(current_x/.5), angle_to_index(current_theta)] = True
        ...
        for move in directions:
            ...
            if valid_move(newNode.Node_x, newNode.Node_y, map.shape, obstacles): # Check that it isn't in an obstacle.
                ...
                node_key = (int(newNode.Node_y / 0.5), int(newNode.Node_x / 0.5), angle_to_index(newNode.Node_theta))
